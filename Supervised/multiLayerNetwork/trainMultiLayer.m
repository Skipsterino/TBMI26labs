...
Wout = [W0,ones(size(Dtraining,1),1)];
Vout = [V0;ones(1,size(Xtraining,1))];
...
H=tanh(Vout*Xtraining);
H(end,:)=1;
Ytraining = Wout*H; 

grad_v = 2/numTraining*((Wout'*(Ytraining-Dtraining)).*(1-tanh(Vout*Xtraining).^2))*Xtraining'; %Calculate the gradient for the output layer
grad_w = 2/numTraining*(Ytraining- Dtraining)*tanh(Vout*Xtraining)';
